    ...
    def render(self, factor=30, key=0):
        ...
        for i in range(self.row):
            for j in range(self.col):
                ...
                x, x_next = (i+1)*factor, (i+2)*factor
                y, y_next = (j+1)*factor, (j+2)*factor
                
                if self.cell[i][j][0]: #top
                    cv2.line(img, (y, x), (y_next, x), (0, 0, 0), 1)
                if self.cell[i][j][1]: #left
                    cv2.line(img, (y, x), (y, x_next), (0, 0, 0), 1)
                if self.cell[i][j][2]: #bottom
                    cv2.line(img, (y, x_next), (y_next, x_next), (0, 0, 0), 1)
                if self.cell[i][j][3]: #right
                    cv2.line(img, (y_next, x), (y_next, x_next), (0, 0, 0), 1)
